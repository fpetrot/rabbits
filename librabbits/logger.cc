...
int Logger::printf(LogLevel lvl, const std::string fmt, ...)
{
    va_list ap;
    ...
    va_start(ap, fmt);
    written = vsnprintf(&m_format_buf[0], m_format_buf.capacity(), fmt.c_str(), ap);
    va_end(ap);
    ...
    stream(lvl) << std::string(&m_format_buf[0]);

    return written;
}


std::ostream & Logger::stream(LogLevel lvl)
{
    if (lvl > m_level) {
